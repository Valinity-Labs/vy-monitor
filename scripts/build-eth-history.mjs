 ...
 *
 * PRICE. Each swap records both what it actually paid (vy/quote) and the POOL'S OWN PRICE after
 * it (`poolPrice`, from the Sync the pool emits with every swap: quote reserve ÷ VY reserve). The
 * live USDC pool is charted at the pool price — what the pool quotes and the web app reads — and
 * the tape shows what each trade paid.
 *
...
async function buildEra(era, head) {
  ...
  console.log(`[${era.id}] Swap + Sync logs ${era.created.toLocaleString('en-US')} -> ${to.toLocaleString('en-US')} ...`);
  const logs = await rpc('eth_getLogs', [{
    address: era.pool, topics: [[SWAP_TOPIC, SYNC_TOPIC]],
    fromBlock: '0x' + era.created.toString(16), toBlock: '0x' + to.toString(16),
  }]);
  if (!Array.isArray(logs)) throw new Error(`[${era.id}] eth_getLogs failed`);
  logs.sort((a, b) => Number(a.blockNumber) - Number(b.blockNumber) || Number(a.logIndex) - Number(b.logIndex));
  const raw = logs.filter((l) => l.topics[0] === SWAP_TOPIC);
  console.log(`  ${raw.length.toLocaleString('en-US')} swaps, ${(logs.length - raw.length).toLocaleString('en-US')} reserve updates`);
  ...
  // THE POOL'S OWN PRICE after each swap. Uniswap V2 emits Sync(reserve0, reserve1) immediately
  // before every Swap, so walking the logs in chain order, the latest Sync seen when a Swap arrives
  // holds exactly the reserves that swap left. token0 is VY in both pools.
  const trades = [];
  let skipped = 0;
  let reserves = null;
  for (const l of logs) {
    if (l.topics[0] === SYNC_TOPIC) { reserves = words(l.data); continue; }
    ...
    const quoteUsd = ethUsdAt ? Number(ethUsdAt(block).toFixed(2)) : 1;
    trades.push({
      era: era.id, block,
      ...
      quoteUsd,
      poolPrice: reserves && reserves[0] > 0n
        ? Number(((Number(reserves[1]) / 10 ** era.quote.decimals) / (Number(reserves[0]) / 1e18) * quoteUsd).toPrecision(10))
        : null,
    });
  }
...
const out = {
  _comment:
    ...
    'legacy VY/WETH pool and the current, LIVE VY/USDC pool. Each swap carries what it paid ' +
    '(vy/quote) and poolPrice, the pool price after it from its Sync event; the legacy era ' +
    'converts ETH->USD with an interpolated Chainlink series.',
